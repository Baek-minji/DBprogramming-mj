<?php
    ...
    if(isset($_GET['by_genre'])){
        $Genres = mysqli_real_escape_string($link,$_GET['by_genre']);
    }else{
        $Genres = mysqslei_real_escape_string($link,$_POST['by_genre']);
    }

    // echo $Genres;
    
    $query = "
    SELECT GrammyYear,Award,Album,Artist
    FROM grammyAlbums 
    WHERE Genre='{$Genres}'
    ORDER BY GrammyYear 
    ";
